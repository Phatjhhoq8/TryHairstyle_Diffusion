"""
HairFusion Inference Script.
Standalone entry point for batch inference.
"""
import os
...
# Initialize backend (sys.path setup for library imports)
import backend  # noqa: F401

from cldm.ddim_hacked import DDIMSampler
from cldm.model import create_model, load_state_dict
from backend.app.utils.image_utils import tensor2img
from backend.app.config import CONFIG_YAML_PATH, DATA_DIR

parser = argparse.ArgumentParser()
parser.add_argument("--config_name", type=str)
parser.add_argument("--model_load_path", type=str)
parser.add_argument("--vae_load_path", type=str, default=None, help="None is sd vae")
...
parser.add_argument("--last_n_blend", type=float, default=None, help="adaptive blending for last n timesteps during inference")


args = parser.parse_args()

# Default data_root_dir to backend/data/test
if args.data_root_dir is None:
    args.data_root_dir = os.path.join(DATA_DIR, "test")
...
def build_config(args, config_path=None):
    ...
    if args is not None:
        config.model.params.validation_config.img_save_dir = args.valid_img_save_dir if hasattr(args, 'valid_img_save_dir') else None
        config.model.params.validation_config.real_dir = args.valid_real_dir if hasattr(args, 'valid_real_dir') else None
    return config
...
with torch.no_grad():
    ...
    if args.config_name is None:
        ...
        config_path = os.path.join("backend", "configs", f"{args.config_name}.yaml")
